...
!DOC !PROG Usage:  mcrdf parameters.prm composition output_prefix [dr Rmax  [mol_labels [nskip[-maxfram]  ]  ]   ] '
!DOC !PROG Arguments::
...
!DOC !PROG                  The columns are :  r count(r) g(r) cnt2(r) g2(r) cnt3(r) g3(r) cnt4(r)' 
!DOC !PROG                            where:'
!DOC !PROG                                       count(r) = number of particles found in [r;r+dr] '
!DOC !PROG                                       g(r) = count(r) / N_total * Vmax / dV  '
!DOC !PROG                                               ( N_total - total number of distances counted)'
!DOC !PROG  '  
!DOC !PROG                                       cnt2(r) = sum_frame sum_ij  1/r_ij^2  where r_ij in [r;r+dr]   '
!DOC !PROG                                       g2(r)  =  1/(4pi rho^2 dr N_frames)  1/<V> cnt2(r)  '
!DOC !PROG  '
!DOC !PROG                                       cnt3(r) = sum_frame 1/V_frame sum_ij  1/r_ij    '
!DOC !PROG                                       g3(r) =  1/(4pi rho^2 dr N_framess) cnt3(r)' 
!DOC !PROG    NOTE !!! ONLY cnt4 (8th column) corresponds to the usual way of collecting the data --> only 8th column should be used for any comparisons                           '
!DOC !PROG  '
      ...
      do j=i,ntypes
         ...
         do k=1,N
            ...
            if ( N_total(offset) > 0 ) then
               rdf1 = cnt( offset ) % ptr(k) / N_total(offset) * Vmax / dV  
            else
               rdf1 = 0d0
            end if
...
if ( iargc() < 3 ) then
   ...
   write(*,*)  '                 The columns are :  r count(r) g(r) cnt2(r) g2(r) cnt3(r) g3(r)' 
   write(*,*)  '                           where:'
   write(*,*)  '                                      count(r) = number of particles found in [r;r+dr] '
   write(*,*)  '                                      g(r) = count(r) / N_total * Vmax / dV  '
   write(*,*)  '                                              ( N_total - total number of distances counted)'
   write(*,*)  ' '  
   write(*,*)  '                                      cnt2(r) = sum_frame sum_ij  1/r_ij^2  where r_ij in [r;r+dr]   '
   write(*,*)  '                                      g2(r)  =  1/(4pi rho^2 dr N_frames)  1/<V> cnt2(r)  '
   write(*,*)  ' '
   write(*,*)  '                                      cnt3(r) = sum_frame 1/V_frame sum_ij  1/r_ij    '
   write(*,*)  '                                      g3(r) =  1/(4pi rho^2 dr N_framess) cnt3(r)' 
   write(*,*)  '                              (i believe that g3(r) is correct, but just read RDF.pdf for details)'
   write(*,*)  ' '
